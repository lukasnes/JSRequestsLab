...
// CODE HERE
function returnButton(evt) { //the callback function should refer to the event of whatever the user is going to do with the object. The evt.target is what determines where the user performed the action.
    evt.target.style.backgroundColor = "#efefef"
    evt.target.style.color = "black"
}
sayHelloButton.addEventListener('mouseout', returnButton)
...
let aniBtn = document.getElementById('animals-button')
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/blahblah').then((res) =>{
...
function queries() {
    axios.get('http://localhost:3000/query-test/?text=Hello').then((res) => {
        console.log(res.data)
    })
}
let queryBtn = document.getElementById('query-button')
queryBtn.addEventListener('click',queries)
